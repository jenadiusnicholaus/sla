...
export const cmsApi = {
  ...
  hero: () => api('/hero/'),
  saveHero: (id, body) =>
    id ? api(`/hero/${id}/`, { method: 'PATCH', body }) : api('/hero/', { method: 'POST', body }),
  gallery: () => api('/gallery/'),
  saveGallery: (id, body) =>
    id ? api(`/gallery/${id}/`, { method: 'PATCH', body }) : api('/gallery/', { method: 'POST', body }),
  ...
  deleteGalleryImage: (id) => api(`/gallery-images/${id}/`, { method: 'DELETE' }),
  programs: () => api('/programs/'),
  saveProgram: (id, body) =>
    id ? api(`/programs/${id}/`, { method: 'PATCH', body }) : api('/programs/', { method: 'POST', body }),
  deleteProgram: (id) => api(`/programs/${id}/`, { method: 'DELETE' }),
  team: () => api('/team/'),
  saveTeam: (id, body) =>
    id ? api(`/team/${id}/`, { method: 'PATCH', body }) : api('/team/', { method: 'POST', body }),
  deleteTeam: (id) => api(`/team/${id}/`, { method: 'DELETE' }),
  values: () => api('/values/'),
  saveValue: (id, body) =>
    id ? api(`/values/${id}/`, { method: 'PATCH', body }) : api('/values/', { method: 'POST', body }),
  ...
  updateContactMessage: (id, body) => api(`/contact-messages/${id}/`, { method: 'PATCH', body }),
  replyContactMessage: (id, body) =>
    api(`/contact-messages/${id}/reply/`, { method: 'POST', body }),
  meetingRequests: (params = '') => api(`/meeting-requests/${params ? `?${params}` : ''}`),
  postMeetingRequest: (body) => api('/meeting-requests/', { method: 'POST', body }),
  updateMeetingRequest: (id, body) => api(`/meeting-requests/${id}/`, { method: 'PATCH', body }),
  postNewsletter: (email) => api('/newsletter/', { method: 'POST', body: { email, source: 'footer' } }),
...
export const qrApi = {
  ...
  trackLink: (code, linkId) => api(`/qr/track-link/${code}/${linkId}/`, { method: 'POST', body: {} }),
